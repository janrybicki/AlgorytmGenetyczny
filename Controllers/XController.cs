    ...
    public class XController : Controller
    {
        ...
        public ActionResult Create(XModel xModel)
        {
            ...
            for (int i = 0; i < xModel.Number; i++)
            {
                ...
                var xReal1 = randomNumber - randomNumber % xModel.Precision;
                ...
                var xReal2Roundend = Math.Abs(xReal2 % xModel.Precision) < xModel.Precision / 2 ? xReal2 - xReal2 % xModel.Precision : xReal2 + Math.Sign(xReal2) * xModel.Precision - xReal2 % xModel.Precision;
                var functionValue = (xReal2Roundend % 1) * (Math.Cos(xReal2Roundend * 20 * Math.PI) - Math.Sin(xReal2Roundend));
                //var functionValueRounded = Math.Abs(functionValue % xModel.Precision) < xModel.Precision / 2 ? functionValue - functionValue % xModel.Precision : functionValue + Math.Sign(functionValue) * xModel.Precision - functionValue % xModel.Precision;
